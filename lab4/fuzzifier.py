...
def soil_wetness(x):
    ...
    return np.array([very_dry, dry, normal, wet, very_wet])

def tank_fullness(x):
    tank_empty = trapmf(x, -0.1, 0, 0, 0.2)
    tank_normal = trapmf(x, 0, 0.2, 0.8, 1)
    tank_overflowing = trapmf(x, 0.9, 1, 1, 1.1)
    return np.array([tank_empty, tank_normal, tank_overflowing])
...
plt.ylabel("Знач. функ. приналежності")
rainfall_was_recent = days_since_rainfall(days)[0]
do_not_water = days_since_too_much_water(days)[0]
